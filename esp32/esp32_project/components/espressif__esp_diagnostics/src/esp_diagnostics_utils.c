 ...
 */
#include "freertos/task.h" // Ensure this header is included
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
...
static void __attribute__((unused)) diag_task_get_bt(esp_diag_task_bt_t *bt_info, XtExcFrame *stack)
{
...
uint32_t esp_diag_task_snapshot_get(esp_diag_task_info_t *tasks, size_t size)
{
    ...
    TaskStatus_t *snapshots = calloc(task_count, sizeof(TaskStatus_t));
    if (!snapshots) {
        return 0;
    }
    uint32_t tcb_size = 0; /* unused */
    uint32_t count = uxTaskGetSystemState(snapshots, task_count, &tcb_size);
    if (count == 0) {
        free(snapshots);
        return 0;
    }
    for (i = 0; i < count; i++) {
        TaskHandle_t handle = snapshots[i].xHandle;
...
#ifndef CONFIG_IDF_TARGET_ARCH_RISCV
        // If backtrace functionality is needed, consult ESP-IDF documentation for alternatives.
#endif /* !CONFIG_IDF_TARGET_ARCH_RISCV */
    }
    free(snapshots);
